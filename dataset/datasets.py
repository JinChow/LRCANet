    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            if args.num_sample > 1:
                ...
                # buffer = self.RandCrop(buffer, args) # For ablation
            ...
            final_buffer, chunk_nb, split_nb = self.LRSL(buffer, test_num_crop=self.test_num_crop, test_num_segment=self.test_num_segment)
            return final_buffer, self.test_label_array[index], sample.split("/")[-1].split(".")[0], chunk_nb, split_nb
        else:
            raise NameError('mode {} unkown'.format(self.mode))

    def LRSL(self, buffer, test_num_crop, test_num_segment):
        frame_list = []
        if isinstance(buffer, list):
            buffer = np.array(buffer)
        for item in self.test_seg:
            chunk_nb, split_nb = item
            if self.sparse_sample:
                ...
                if buffer.shape[1] >= buffer.shape[2]:
                    buffer = buffer[temporal_start::test_num_segment,
                ...
                spatial_start = int(split_nb * spatial_step)
                if buffer.shape[1] >= buffer.shape[2]:
                    buffer1 = buffer[temporal_start:temporal_start +
                                                    self.clip_len,
                              spatial_start:spatial_start +
                                            self.short_side_size, :, :]
                else:
                    buffer1 = buffer[temporal_start:temporal_start +
                              ...
                              spatial_start:spatial_start +
                                            self.short_side_size, :]
                buffer1 = self.data_transform(buffer1)
                frame_list.append(buffer1)
        buffer_all = torch.stack(frame_list, dim=0)
        return buffer_all, chunk_nb, split_nb

    def RandCrop(self, buffer, args):
        input_mean = [0.485, 0.456, 0.406]
        input_std = [0.229, 0.224, 0.225]
        div = True
        roll = False
        normalize = GroupNormalize(input_mean, input_std)
        ablation_randcrop = GroupMultiScaleCrop(args.input_size,
                                                [1, .875, .75, .66])
